...
from app.core.send_mail import send_manual_vat_email, send_vat_report_email_safely, send_email, send_quarter_issues_email
...
async def extract_file_headers(file: UploadFile) -> tuple[list[str], pd.DataFrame, bytes]:
    try:
        ...
        if 'order_date' in df.columns:
            ...
            if invalid_quarter_rows:
                data_issues.append({
                    ...
                    "total_rows": len(df),
                    "percentage": round((len(invalid_quarter_rows) / len(df)) * 100, 2),
    ...
    try:
        ...
        for issue in issues:
            issues_rows.append({
                ...
                "Missing Count": issue.get("total_missing") or issue.get("invalid_count", ""),
        ...
        for issue in issues:
            ...
            if issue["issue_type"] == "MISSING_DATA":
                ...
                for row_info in issue.get("invalid_rows", []):
                    try:
                        row_num = row_info["row"] if isinstance(row_info, dict) else int(row_info)
                        ws_data.cell(row=row_num, column=col_idx).fill = red_fill
                    except Exception:
                        continue
...
async def send_manual_review_admin_email(
    ...
    try:
        ...
        if isinstance(enrichment_result, dict) and enrichment_result.get("status") == "manual_review_required":
        ...
        if df is None or df.empty:
            raise HTTPException(status_code=400, detail="Processed VAT data not found or is empty")
        ...
        def rename_headers(dataframe):
            if dataframe is None or dataframe.empty:
                return dataframe
            rename_map = {col: header_labels[col] for col in dataframe.columns if col in header_labels}
            if rename_map:
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Could not send manual review email: {str(e)}")
        ...
        for issue in data_issues:
            if issue.get('issue_type') == 'INVALID_QUARTER':
                quarter_issues.append({
                    'column': issue.get('header_label', issue.get('original_column', 'Unknown')),
                    'invalid_count': issue.get('invalid_count', 0),
                    'percentage': issue.get('percentage', 0),
                    'invalid_rows': issue.get('invalid_rows', [])  # Include all invalid rows
                })
...
"""
        ...
        rename_map = {col: header_labels.get(col, col) for col in df_for_email.columns if col in header_labels}
        if rename_map:
            df_for_email.rename(columns=rename_map, inplace=True)

        # Format dates for display
        for col in df_for_email.columns:
            if "date" in col.lower() and pd.api.types.is_datetime64_any_dtype(df_for_email[col]):
        ...
        with pd.ExcelWriter(excel_stream, engine='openpyxl') as writer:
            ...
            if invalid_row_numbers:
                ...
                legend_cell.fill = PatternFill(start_color="FFFFCC", end_color="FFFFCC", fill_type="solid")  # Light yellow
                ...
                print(f"Debug: Highlighted {len(invalid_row_numbers)} rows: {sorted(invalid_row_numbers)}")
                print(f"Debug: Adjusted Excel rows: {sorted([r + 1 for r in invalid_row_numbers])}")
            
            # Create issues summary sheet
            if quarter_issues:
                issues_data = []
                for issue in quarter_issues:
                    for row_info in issue['invalid_rows']:  # Include all invalid rows in Excel
                        if isinstance(row_info, dict):
                            issues_data.append({
                                'Column': issue['column'],
                                'Row Number': row_info.get('row', 'N/A'),
                                'Invalid Value': row_info.get('value', 'N/A'),
                                'Issue Description': row_info.get('issue', 'Outside quarter range')
                            })
                ...
                if issues_data:
                    ...
                    for column in issues_worksheet.columns:
                        ...
                        issues_worksheet.column_dimensions[column_letter].width = adjusted_width
